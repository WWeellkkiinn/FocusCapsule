...
def test_validate_config_accepts_normal_and_disabled_interval_configs() -> None:
    assert validate_config(SessionConfig(25, 3, 5, 10, 5, True)) == []
    assert validate_config(SessionConfig(25, 0, 0, 10, 5, True)) == []
    assert validate_config(SessionConfig(25, 0.1, 0.2, 10, 5, True)) == []


def test_validate_config_rejects_interval_configuration_errors() -> None:
    zero_min_errors = validate_config(SessionConfig(25, 0, 5, 10, 5, True))
    negative_min_errors = validate_config(SessionConfig(25, -1, 5, 10, 5, True))
    max_not_less_errors = validate_config(SessionConfig(5, 0.1, 5.0, 10, 5, True))
...
def test_validate_config_rejects_non_finite_interval_values() -> None:
    nan_errors = validate_config(SessionConfig(25, float("nan"), 5, 10, 5, True))
    inf_errors = validate_config(SessionConfig(25, 1, math.inf, 10, 5, True))
...
def test_validate_config_can_report_multiple_errors_together() -> None:
    errors = validate_config(SessionConfig(4, 6, 5, 200, 0, True))

    assert len(errors) >= 3
    assert "结束后休息时长必须在 1~60 分钟之间" in errors
