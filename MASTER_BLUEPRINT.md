...
- **核心函数/模块：**
  - `scripts/check_tcl.ps1`
    - 使用 `C:\Users\asd13\anaconda3\envs\FocusCapsule\python.exe` 输出：
  ...
  - `scripts/build_win.bat`
    - 强制使用绝对路径解释器：`C:\Users\asd13\anaconda3\envs\FocusCapsule\python.exe`
    - 构建前删除：`build`、`dist`、`FocusCapsule.spec`
    - 执行：
      - `python -m pip install -r requirements.txt`
      - `python -m PyInstaller --noconfirm --onefile --windowed --name FocusCapsule main.py`
- **约束条件：**
  - 禁止使用 `conda run` 打包。
...
- [x] **Task 3:** 在 Windows 侧执行环境校验：
  - `C:\Users\asd13\anaconda3\Scripts\conda.exe list -n FocusCapsule tcl tk python`
  - `powershell -File scripts/check_tcl.ps1`
- [x] **Task 4:** 若版本不一致，统一环境版本（推荐 `python 3.11 + tcl/tk 8.6.15`），并再次通过 `check_tcl.ps1`。
- [x] **Task 5:** 使用 `scripts/build_win.bat` 重新打包，生成 `dist/FocusCapsule.exe`。
...
- 默认目标环境：`C:\Users\asd13\anaconda3\envs\FocusCapsule`。
- 默认继续使用 `PyInstaller --onefile --windowed`。
- 默认 Windows 10/11。
